...
class NonInspectionController extends Controller
{
    /**
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($date) && !empty($attr['type'])) {
            switch ($attr['type']) {
                case 'mine':
                    $data = NonInspection::with(['files'])->orWhere(function ($query) use ($attr, $author) {
                        $query->where('created_by', $author);
                        $query->orWhere('marketing_officer_id', $author);
                    })->byCreatedDate($date);
                    break;
                case 'all':
                    $data = NonInspection::with(['files'])->byCreatedDate($date);
                    break;
                default:
                    break;
            }
        ...
        return inertia('Inspection/NonInspectionPage', [
            'branches' => $branches,
            'mos' => $mos,
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return back()->with([
                'type' => 'success',
                'message' => 'Non Inspection has been created',
            ]);
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            $inspect = NonInspection::find($id);
            $inspect->update($attr);

            return back()->with([
                'type' => 'success',
                'message' => 'Non Inspection has been updated',
            ]);
        } catch (\Illuminate\Database\QueryException $ex) {
            return back()->with([
                'type' => 'error',
    ...
    public function destroy(string $id)
    {
        ...
        try {
            ...
            return back()->with([
                'type' => 'success',
                'message' => 'Non Inspection has been deleted',
            ]);
        } catch (\Illuminate\Database\QueryException $ex) {
            return back()->with([
                'type' => 'error',
