...
export default function Index() {
    ...
    const onDelete = (id: any) => {
        router.delete(route('schedule-inspection.destroy', id));
    }
        ...
        return policies.map((value: any) => {
            return {
                ...
                'action': <Group justify='end'>
                    <HasPermission permission='scheduleinspection.delete' author={value.created_by}>
                        <DeleteButton onDelete={() => onDelete(value.id)} />
                    </HasPermission>
                    <ButtonModal buttonOptions={{ children: 'View' }} modalOptions={{ title: 'View Schedule Inspection', size: 'xl' }}>
                        <ViewScheduleInspection id={value.id} />
                    </ButtonModal>
                    <ButtonModal buttonOptions={{ children: 'Edit' }} modalOptions={{ title: 'Edit Inspection', size: 'xl' }}>
                        <AddSceduleInspection values={value} type={'inspection'} />
                    </ButtonModal>
                </Group>,
            };
        })
    ...
    return (
        <div>
            <AppCard title='Inspection List' rightComponent={
                permissions.includes('policy.add') && <AddButtonModal title='Add Schedule Inspection'>
                    <AddSceduleInspection />
                </AddButtonModal>
            }>
...
Index.layout = (page: any) => <AdminLayout children={page} title='Manage Schedule Inspection' />;
