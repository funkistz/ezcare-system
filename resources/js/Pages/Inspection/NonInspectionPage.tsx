...
import AddNonInspection from './components/AddNonInspection';
...
export default function Index() {
    ...
    const onDelete = (id: any) => {
        router.delete(route('non-inspection.destroy', id));
    }
        ...
        return policies.map((value: any) => {
            return {
                ...
                'action': <Group justify='end'>
                    <HasPermission permission='noninspection.delete' author={value.created_by}>
                        <DeleteButton onDelete={() => onDelete(value.id)} />
                    </HasPermission>
                    <ButtonModal buttonOptions={{ children: 'View' }} modalOptions={{ title: 'View Non-Inspection', size: 'xl' }}>
                        <ViewInspection id={value.id} type={'non-inspection'} />
                    </ButtonModal>
                    <ButtonModal buttonOptions={{ children: 'Edit' }} modalOptions={{ title: 'Edit Non-Inspection', size: 'xl' }}>
                        <AddNonInspection values={value} type={'non-inspection'} />
                    </ButtonModal>
                </Group>,
            };
        })
    ...
    return (
        <div>
            <AppCard title='Inspection List' rightComponent={
                permissions.includes('policy.add') && <AddButtonModal title='Add Non-Inspection'>
                    <AddNonInspection />
                </AddButtonModal>
            }>
...
Index.layout = (page: any) => <AdminLayout children={page} title='Manage Non-Inspection' />;
