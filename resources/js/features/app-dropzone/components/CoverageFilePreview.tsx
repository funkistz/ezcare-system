...
export default function CoverageFilePreview({ file, type, display = 'card' }: { file: any, type: any, display?: any }) {
    ...
    console.log('preview >>>>', file, new Date(file.activate_date));
    ...
    return (
        <>
            ...
            {display == 'card' && <Tooltip label={file.name}><Card withBorder radius="md" p='xs' className={classes.wrapper} onClick={onPreview} w={200}>
                ...
                <Text fz={13} fw={700} lineClamp={2} truncate="end">Activate Date : {formatDate(file.activate_date)}</Text>
